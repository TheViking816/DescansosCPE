...
-- ============================================
-- Objetivo:
-- - Login/registro con contraseña usando Supabase Auth
...
-- 2) Si ya tienes tablas `usuarios/ofertas` antiguas, revisa la sección "LEGACY".
...
-- Seed inicial (pásame el resto y lo añadimos aquí)
...
-- Backfill/upgrade si la tabla ya existía sin la columna
...
-- Intentar añadir FK si no existe (si falla por datos, corrige y reintenta)
DO $$
BEGIN
  ALTER TABLE public.usuarios
...
-- Ofertas: lectura para autenticados; mutación solo propias
